...
(function setupThemeSwitch() {
  ...
  const saved = body.dataset.theme || localStorage.getItem("theme") || "light";
...
function parseCsv(text) {
  ...
  if (!lines.length) { globalTrades = []; globalBySymbol = {}; return; }
...
function buildStats(trades) {
  ...
  for (const t of trades) {
    ...
    if (p > 0) { profitTrades++; grossProfit += p; curConsecLoss = 0; }
      ...
      lossTrades++; grossLoss += -p; curConsecLoss++;
  ...
  return {
    totalTrades, grossProfit, grossLoss, profitTrades, lossTrades,
    winRate, lossRate, avgWin, avgLoss, expectancy,
    profitFactor: pf, maxDrawdown: maxDD, maxConsecLoss,
  };
}
...
function resetView() {
  globalTrades = []; globalBySymbol = {}; globalEAKey = "SMA";
  mfeMaeMode = "pips"; cumulativeMode = "all";
  ...
  symbolCumulativeChart = symbolWeekdayProfitChart = symbolWeekdayCountChart =
    symbolHourlyProfitChart = symbolHourlyCountChart = null;
  const hideIds = [
    "summaryCardsSection", "symbolSection",
    "symbolDetailSection", "swotSection", "martinSection",
  ];
  hideIds.forEach((id) => {
  ...
  const clearIds = [
    "symbolButtons", "symbolMiniCharts", "symbolStats", "martinTables",
    "swotST", "swotS", "swotSW", "swotT", "swotW",
    "swotOT", "swotO", "swotOW", "eaCenterAnalysis",
  ];
  clearIds.forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = "";
  ...
  const summaryDefaults = {
    growthValue: "0 %", growthPeriod: "",
    radarProfitTrades: "", radarLossTrades: "", radarDepositLoad: "",    radarMaxDD: "", radarActivity: "",
    equityValue: "0.00", profitValue: "0.00", initialDepositValue: "0.00",
  };
...
function renderSummaryCards(acc) {
  ...
  const periodDays = acc.firstTime && acc.lastTime
    ? Math.max(1, Math.round((acc.lastTime.getTime() - acc.firstTime.getTime()) / (1000 * 3600 * 24)))
    : 0;
...
function renderSymbolButtons() {
  ...
  allBtn.innerHTML = `All Symbols &nbsp;<span>${allNet.toFixed(0)}</span>`;
  allBtn.onclick = () => {
  ...
  symbols.forEach((sym) => {
    ...
    btn.innerHTML = `${sym} &nbsp;<span>${net.toFixed(0)}</span>`;
    btn.onclick = () => {
      [...container.querySelectorAll(".symbol-btn")].forEach((b) => b.classList.remove("active"));
...
function renderSymbol(symbol) {
  ...
  document.getElementById("symbolTitle").textContent =
    symbol === "ALL" ? "5. Symbol 深入分析 📊 – All Symbols" : `5. Symbol 深入分析 📊 – ${symbol}`;
...
function renderSymbolStats(stats) {
  ...
  el.innerHTML = `
    <p>單數: ${stats.totalTrades} &nbsp; 勝率: ${(stats.winRate * 100).toFixed(1)}% &nbsp; 淨盈利: ${net.toFixed(2)} &nbsp; PF: ${stats.profitFactor === Infinity ? "∞" : stats.profitFactor.toFixed(2)}</p>
    <p>期望值/單: ${stats.expectancy.toFixed(2)} &nbsp; Max DD: ${stats.maxDrawdown.toFixed(2)} &nbsp; 最大連虧: ${stats.maxConsecLoss}</p>
  `;
}
...
function buildMartinForSymbol(trades) {
  ...
  for (const t of trades) {
    ...
    const m = map[key];
    m.tradeCount++; m.sumProfit += t.netProfit; m.sumPips += t.netPips;
    if (t.netProfit > 0) { m.winCount++; if (m.minWinProfit === null || t.netProfit < m.minWinProfit) m.minWinProfit = t.netProfit; }
  ...
  for (const key of Object.keys(bySide)) {
    ...
    const rowsOut = [];
    for (const r of arr) {
      totalProfit += r.sumProfit; totalPips += r.sumPips; totalTrades += r.tradeCount;
      levelIndex++; cum += r.sumProfit;
    ...
    if (firstPositiveLevel !== null) {
      if (martinSummary.firstPositiveLevel === null || firstPositiveLevel < martinSummary.firstPositiveLevel)
...
function renderMartinTables(symbol, tablePerSide) {
  ...
  tablePerSide.forEach((block) => {
    ...
    title.innerHTML = `<span>${block.symbol} - ${block.side}</span> <span class="${totalClass}">TOTAL Profit: ${block.totalProfit.toFixed(2)}, Trades: ${block.totalTrades}</span>`;
...
function addMiniChartCard(container, label, trades) {
  ...
  title.innerHTML = `${label} <span>${net.toFixed(0)}</span>`;
  div.appendChild(title);
  container.appendChild(div);
  let cum = 0;
  const points = [];
  trades.forEach((t) => { cum += t.netProfit; points.push(cum); });
...
function renderMfeMaeHoldingCharts(trades) {
  ...
  mfeChart = new Chart(mfeCtx.getContext("2d"), { type: "scatter", data: { datasets: [{ label: "MFE vs Result", data: mfeData, backgroundColor: mfeData.map((d) => d.c) }] }, options: { parsing: false, plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: xTitle } }, y: { title: { display: true, text: "MFE (pips)" } } } } });
  maeChart = new Chart(maeCtx.getContext("2d"), { type: "scatter", data: { datasets: [{ label: "MAE vs Result", data: maeData, backgroundColor: maeData.map((d) => d.c) }] }, options: { parsing: false, plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: xTitle } }, y: { title: { display: true, text: "MAE (pips)" } } } } });
  holdingChart = new Chart(holdCtx.getContext("2d"), { type: "scatter", data: { datasets: [{ label: "Holding Time vs Result", data: holdData, backgroundColor: holdData.map((d) => d.c) }] }, options: { parsing: false, plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: xTitle } }, y: { title: { display: true, text: "Holding Time (days)" } } } } });
}
...
function renderSymbolExtraCharts(symbol, trades) {
  ...
  if (symbol === "ALL" && cumulativeMode === "separate") {
    ...
    Object.entries(grouped).forEach(([symKey, arr]) => {
      ...
      arr.forEach((t) => { cum += t.netProfit; data.push(cum); });
      if (data.length > maxLen) maxLen = data.length;
      const c = baseColors[colorIndex++ % baseColors.length];
      datasets.push({ label: symKey, data, borderColor: c, fill: false, pointRadius: 0, tension: 0.15 });
    });
    const labels = Array.from({ length: maxLen }, (_, i) => i + 1);
    symbolCumulativeChart = new Chart(cumCtx2d, { type: "line", data: { labels, datasets }, options: { plugins: { legend: { display: true } }, scales: { x: { title: { display: true, text: "Trade Index (per Symbol)" } }, y: { title: { display: true, text: "Profit" } } } } });
  } else {
    let cum = 0; const cumLabels = []; const cumData = [];
    sorted.forEach((t, idx) => { cum += t.netProfit; cumLabels.push(idx + 1); cumData.push(cum); });
    symbolCumulativeChart = new Chart(cumCtx2d, { type: "line", data: { labels: cumLabels, datasets: [{ label: "Cumulative Profit", data: cumData, borderColor: "#2563eb", fill: false, pointRadius: 0, tension: 0.15 }] }, options: { plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: "Trade Index" } }, y: { title: { display: true, text: "Profit" } } } } });
  }
  ...
  symbolWeekdayProfitChart = new Chart(wdProfitCtx.getContext("2d"), { type: "bar", data: { labels: weekdayNames, datasets: [{ label: "Profit", data: weekdayProfit, backgroundColor: weekdayProfit.map((v) => v >= 0 ? "#22d3ee" : "#ef4444") }] }, options: { plugins: { legend: { display: false } }, scales: { y: { title: { display: true, text: "Profit" } } } } });
  symbolWeekdayCountChart = new Chart(wdCountCtx.getContext("2d"), { type: "bar", data: { labels: weekdayNames, datasets: [{ label: "Count", data: weekdayCount, backgroundColor: "#6366f1" }] }, options: { plugins: { legend: { display: false } }, scales: { y: { title: { display: true, text: "Trades" }, beginAtZero: true } } } });
  ...
  symbolHourlyProfitChart = new Chart(hrProfitCtx.getContext("2d"), { type: "bar", data: { labels: hourLabels, datasets: [{ label: "Profit", data: hourlyProfit, backgroundColor: hourlyProfit.map((v) => v >= 0 ? "#22d3ee" : "#ef4444") }] }, options: { plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: "Hour" } }, y: { title: { display: true, text: "Profit" } } } } });
  symbolHourlyCountChart = new Chart(hrCountCtx.getContext("2d"), { type: "bar", data: { labels: hourLabels, datasets: [{ label: "Count", data: hourlyCount, backgroundColor: "#3b82f6" }] }, options: { plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: "Hour" } }, y: { title: { display: true, text: "Trades" }, beginAtZero: true } } } });
}
...
function renderSwot(swot) {
  if (!swot) return;
  document.getElementById("swotST").innerHTML = "<b>ST</b><br>" + swot.ST.join("<br>");
  document.getElementById("swotS").innerHTML = "<b>S</b><br>" + swot.S.join("<br>");
  document.getElementById("swotSW").innerHTML = "<b>SW</b><br>" + swot.SW.join("<br>");
  document.getElementById("swotT").innerHTML = "<b>T</b><br>" + swot.T.join("<br>");
  document.getElementById("swotW").innerHTML = "<b>W</b><br>" + swot.W.join("<br>");
  document.getElementById("swotOT").innerHTML = "<b>OT</b><br>" + swot.OT.join("<br>");
  document.getElementById("swotO").innerHTML = "<b>O</b><br>" + swot.O.join("<br>");
  document.getElementById("swotOW").innerHTML = "<b>OW</b><br>" + swot.OW.join("<br>");
